...
/* —— Viewport height: stable across browsers, refresh on rotate —— */
...
function readViewportSize(){
  ...
  if(vv && vv.height){
    /* Use the smaller height so mobile URL bars don't leave clipped areas */
    h=Math.min(h || vv.height, Math.round(vv.height));
    if(vv.width) w=Math.min(w || vv.width, Math.round(vv.width));
  }
  return {
...
function markCompactScreens(){
  const vh=readViewportSize().h;
  document.querySelectorAll("#invitation .screen").forEach(screen=>{
    const content=screen.querySelector(".host-sheet, .content, .event-content") || screen;
    const need=content.scrollHeight + 24;
    screen.classList.toggle("is-compact", need > vh * 0.98);
  });
}
...
function scheduleFitScreens(force){
  ...
  window.requestAnimationFrame(markCompactScreens);
}

syncAppViewport(true);
window.addEventListener("orientationchange", ()=>{
  appHLocked=false;
  window.setTimeout(()=>scheduleFitScreens(true), 150);
  window.setTimeout(()=>scheduleFitScreens(true), 450);
});
window.addEventListener("resize", ()=>{
  /* Only re-fit when width changes a lot (rotate / split screen), not URL-bar jitter */
  const w=readViewportSize().w;
  const prev=parseInt(getComputedStyle(document.documentElement).getPropertyValue("--app-w"), 10) || 0;
  if(Math.abs(w - prev) > 40){
    appHLocked=false;
    scheduleFitScreens(true);
  }
}, {passive:true});
window.addEventListener("pageshow", ()=>scheduleFitScreens(true));
window.addEventListener("load", ()=>scheduleFitScreens(true));
window.setTimeout(()=>scheduleFitScreens(true), 100);
window.setTimeout(()=>scheduleFitScreens(true), 600);
