/* Old cached shells → force latest emerald-gold gate pack */
(function(){
  try{
    if(!document.querySelector("#openGate .language-btn[data-open-lang]")){
      location.replace("index.html?v=79-gate-" + Date.now());
      return;
    }
  }catch(_e){}
})();
...
function fitAllPages(){
  ...
  /* Gate titles must stay readable — never scale/clip the welcome sheet */
  const gate=document.querySelector(".welcome-gate.emerald-gate, .emerald-gate");
  if(gate){
    const sheet=gate.querySelector(".gate-sheet, .emerald-gate-sheet");
    resetFitTarget(sheet);
    gate.classList.remove("is-fitted","is-compact","is-scrollable");
    gate.style.overflow="";
    gate.style.height="";
    gate.style.minHeight="";
    gate.style.maxHeight="";
  }
...
function splitWords(root){
  ...
  /* Word-split + foil clip doubles single-word event titles (Nikah/Reception) */
  if(root.classList.contains("title-hero")) return;
...
const I18N={
  en:{
    ...
    host_hi_date:"",
    host_hi_day:"",
    host_invite:"cordially invite you to celebrate<br>the sacred Nikah <span>&amp;</span> Reception of",
    host_note:"YOUR PRESENCE WILL BE AN<br>HONOUR AND A BLESSING.<br><i>INSHA’ALLAH.</i>",
    gate_sub:"Rooted in Faith • Bound by Love",
    gate_nikah_row:"<b>Nikah</b><span>27 September 2026</span>",
    gate_walima_row:"<b>Reception</b><span>29 September 2026</span>",
    ...
    open_maps:"OPEN IN MAP",
    nikah_maps:"OPEN IN MAP",
    walima_maps:"OPEN IN MAP",
    maps_or:"OR",
    ...
    walima_qr:"SCAN FOR RECEPTION LOCATION",
    walima_qr_alt:"QR code for Reception venue",
    ...
    closing_walima:"Reception 29 Sep",
  ...
  hi:{
    ...
    host_eyebrow:"",
    host_istikbal:"आपका इस्तेक़बाल करते हैं।",
    host_hi_title:"",
    host_families:"आतार <em>&amp;</em> शेख",
    host_invite:"आपको दिल से दावत देते हैं<br>मुबारक निकाह <span>&amp;</span> वलीमा में",
    host_note:"",
    host_hi_date:"",
    host_hi_day:"",
    ...
    open_maps:"MAP में खोलें",
    nikah_maps:"MAP में खोलें",
    maps_or:"या",
    nikah_qr:"निकाह का लोकेशन देखने के लिए स्कैन करें",
    ...
    walima_maps:"MAP में खोलें",
    walima_qr:"वलीमा का लोकेशन देखने के लिए स्कैन करें",
...
let openGate=document.getElementById("openGate");
let openGateHint=openGate ? openGate.querySelector(".open-gate-hint") : null;
...
function musicSrc(){
  try{
    return new URL("assets/bg-music.mp3?v=85", window.location.href).href;
  }catch(e){
    return "assets/bg-music.mp3?v=85";
  }
}

function pinLangSwitchAfterOpen(){
  /* Top English/Hindi switch removed — language chosen on gate only */
}

function hideOpenGate(){
  if(!openGate) openGate=document.getElementById("openGate");
  if(!openGate) return;
  clearReturnToGateTimer();
  pinLangSwitchAfterOpen();
  openGate.classList.remove("is-active-gate");
  openGate.classList.add("is-gone");
  openGate.style.removeProperty("opacity");
  openGate.style.removeProperty("visibility");
  openGate.style.removeProperty("pointer-events");
  openGate.style.removeProperty("display");
  openGate.style.removeProperty("z-index");
  openGate.style.removeProperty("position");
...
let returnToGateTimer=0;
function clearReturnToGateTimer(){
  if(returnToGateTimer){
    window.clearTimeout(returnToGateTimer);
    returnToGateTimer=0;
  }
}

function scheduleReturnToGate(delayMs){
  clearReturnToGateTimer();
  returnToGateTimer=window.setTimeout(()=>{
    returnToGateTimer=0;
    restoreOpenGate();
  }, Math.max(300, delayMs || 800));
}

let restoringGate=false;

function restoreOpenGate(fromPopState){
  if(restoringGate && !fromPopState) return;
  restoringGate=true;
  try{
    clearReturnToGateTimer();
    clearAutoTimer();
    ...
    if(!openGate) openGate=document.getElementById("openGate");
    if(openGate){
      if(!openGate.parentNode){
        const main=document.getElementById("invitation");
        document.body.insertBefore(openGate, main || null);
      }
      openGate.classList.remove("is-gone");
      openGate.classList.add("is-active-gate");
      openGate.style.setProperty("opacity", "1", "important");
      openGate.style.setProperty("visibility", "visible", "important");
      openGate.style.setProperty("pointer-events", "auto", "important");
      openGate.style.setProperty("display", "flex", "important");
      openGate.style.setProperty("z-index", "500", "important");
      document.body.classList.add("gate-locked");
    }

    setHomeBackVisible(false);

    /* Popstate already landed on gate entry — don't replaceState (that broke Back on pasted URLs) */
    if(!fromPopState){
      try{
        const path=location.pathname + location.search;
        if(history.state && history.state.weddingView === "invite"){
          history.back();
        }else{
          history.replaceState({ weddingView:"gate" }, "", path);
        }
      }catch(_e){}
    }
    scheduleFitScreens();
...
/* Browser back → Wedding Invitation / जश्ने शादी gate (not empty tab) */
function weddingHistoryPath(){
  return location.pathname + location.search;
}
function markHistoryGate(){
  try{
    history.replaceState({ weddingView:"gate" }, "", weddingHistoryPath());
  }catch(_e){}
}
function markHistoryInvite(){
  try{
    const path=weddingHistoryPath();
    /* Always: current entry = gate, then push invite on top.
       Paste/new-tab pe pehla Back empty na jaye — gate neeche rahe. */
    history.replaceState({ weddingView:"gate" }, "", path);
    history.pushState({ weddingView:"invite" }, "", path);
  }catch(_e){}
}
function onWeddingPopState(ev){
  const view=ev && ev.state && ev.state.weddingView;
  if(view === "invite") return;
  /* Back from invite (or any non-invite) → show language gate */
  restoreOpenGate(true);
}
...
function warmWebAudioBuffer(){
  /* Decode in background for later resume — never block first audible play */
  const ctx=unlockAudioContext();
...
function startMusicFromGesture(){
  ...
  /* Play HTML audio immediately on tap — waiting for WebAudio decode caused ~2s silence */
  if(bgMusic){
    try{ bgMusic.muted=false; }catch(e){}
  }
  const p=playHtmlAudio();
  warmWebAudioBuffer();
  return p.then((ok)=>{
    if(ok) return true;
    /* Fallback only if HTML play was blocked */
...
function snapToScreenIndex(i){
  const screen=autoScreens[i];
  if(!screen) return;
  const y=Math.max(0, Math.round(screen.offsetTop || 0));
...
function resumeAutoScrollAfterPause(){
  ...
  if(musicEndedAtBottom) return;
  manualScrollMode=false;
  if(autoOn) return;
  autoOn=true;
  autoIndex=nearestScreenIndexSafe();
  snapToScreenIndex(autoIndex);
  ignoreInterruptUntil=Date.now() + 900;
  scheduleFitScreens();
...
/* Touch/wheel: hold current page ~3s, then auto-scroll continues (no permanent manual mode) */
function pauseAutoScrollBriefly(holdMs){
  if(!inviteOpened || !autoScreens.length) return;
  if(document.body.classList.contains("gate-locked")) return;
  if(musicEndedAtBottom) return;
  if(Date.now() < ignoreInterruptUntil && autoOn) return;

  const hold=Math.max(1000, holdMs || 3000);
  autoOn=false;
  manualScrollMode=false;
  clearAutoTimer();
  cancelAutoAnim();
  ...
  autoIndex=nearestScreenIndexSafe();
  snapToScreenIndex(autoIndex);
  ignoreInterruptUntil=Date.now() + 500;
  scheduleFitScreens();
...
function stopAutoScroll(){
  ...
  cancelAutoAnim();
  clearAutoPauseTimer();
  /* Left before last page → do not jump to gate */
  if(autoIndex < autoScreens.length - 1){
    clearReturnToGateTimer();
  }
  scheduleFitScreens();
...
function softScrollTo(top, duration, onDone){
  ...
  function frame(now){
    ...
    if(!autoOn){
      autoRaf=0;
      /* If almost at target (last pages), still finish callback so end→gate can run */
      if(p >= 0.85){
...
function finishAutoAtEnd(){
  /* End of auto show — return to welcome gate (do not rely on autoTimer alone) */
  autoOn=false;
  clearAutoTimer();
  cancelAutoAnim();
  clearAutoPauseTimer();
  ...
  stopMusic();
  releaseScreenWakeLock();
  scheduleReturnToGate(900);
}

function armLastPageReturn(){
  /* Survives stopAutoScroll clearing autoTimer */
  const wait=dwellMsForIndex(Math.max(0, autoScreens.length - 1));
  scheduleReturnToGate(wait + 400);
}
...
function scheduleAfterDwell(){
  ...
  autoTimer=window.setTimeout(()=>{
    ...
    goToScreen(next, ()=>{
      if(!autoOn){
        /* Interrupted mid-move — if we reached last screen area, still arm return */
        if(next >= autoScreens.length - 1) armLastPageReturn();
        return;
      }
...
function onUserWantManual(e){
  ...
  const t=e && e.target;
  if(t && t.closest && (t.closest("#openGate") || t.closest("#homeBackBtn") || t.closest(".home-back-btn") || t.closest(".gold-btn") || t.closest("a") || t.closest(".language-btn") || t.closest(".qr-card"))) return;

  if(e.type === "wheel"){
    pauseAutoScrollBriefly(3000);
    return;
  }
...
function openInvitation(e){
  ...
  ignoreInterruptUntil=Date.now() + 2200;
  window.setTimeout(()=>{
    startAutoScroll();
    scheduleFitScreens();
  }, 600);
...
function setHomeBackVisible(on){
  if(!homeBackBtn) return;
  if(on){
    homeBackBtn.hidden=false;
    homeBackBtn.classList.add("is-visible");
  }else{
    homeBackBtn.classList.remove("is-visible");
    homeBackBtn.hidden=true;
  }
}
if(homeBackBtn){
  const goHome=(ev)=>{
    ...
    restoreOpenGate(false);
  };
  homeBackBtn.addEventListener("click", goHome);
  homeBackBtn.addEventListener("pointerdown", (ev)=>{ ev.stopPropagation(); }, {passive:true});
  homeBackBtn.addEventListener("touchstart", (ev)=>{ ev.stopPropagation(); }, {passive:true});
}

if(openGate){
  document.body.classList.add("gate-locked");
  markHistoryGate();
  window.addEventListener("popstate", onWeddingPopState);
  openGate.querySelectorAll("[data-open-lang]").forEach(btn=>{
    const go=(ev)=>{
      const lang=btn.getAttribute("data-open-lang");
      openWithLang(lang, ev);
    };
...
/* —— Soft floral petal interaction (premium, page-selective) —— */
(function initWeddingFloralInteraction(){
  if(reduceMotion) return;

  const ALLOWED = new Set(["host", "couple", "nikah", "walima"]);
  const BURST = {
    host: { min:14, max:22 },
    couple: { min:14, max:22 },
    nikah: { min:16, max:24 },
    walima: { min:16, max:24 }
  };
  const EDGE_X = 0.22;
  const EDGE_Y = 0.20;
  const MAX_ACTIVE = 72;
  ...
  function screenKey(screen){
    if(!screen) return "";
    if(screen.id === "host" || screen.classList.contains("host")) return "host";
    if(screen.classList.contains("couple")) return "couple";
    if(screen.classList.contains("nikah")) return "nikah";
    if(screen.classList.contains("walima")) return "walima";
    return "";
  }

  function ensureLayer(screen){
    let layer = screen.querySelector(":scope > .floral-petal-layer");
    if(!layer){
      layer = document.createElement("div");
      layer.className = "floral-petal-layer";
  ...
  function visibleAllowedScreen(){
    const screens = document.querySelectorAll("#invitation > .screen.host, #invitation > .screen.couple, #invitation > .screen.event.nikah, #invitation > .screen.event.walima");
    const mid = (window.innerHeight || 1) * 0.45;
    let best = null, bestScore = 0;
    screens.forEach(s => {
      const r = s.getBoundingClientRect();
      const visible = Math.min(r.bottom, window.innerHeight) - Math.max(r.top, 0);
      if(visible < 80) return;
      const score = visible - Math.abs((r.top + r.height / 2) - mid) * 0.15;
      if(score > bestScore){
        bestScore = score;
        best = s;
      }
  ...
  function isBlockedTarget(t){
    ...
    if(t.closest("#openGate, #homeBackBtn, .home-back-btn, .language-btn, .gold-btn, a, button, .qr-card, .qr-card img, input, textarea, select, label, .lang-switch, audio")) return true;
    return false;
  }

  function edgeSide(clientX, clientY){
    const w = window.innerWidth || 1;
    const h = window.innerHeight || 1;
    const x = clientX / w;
    const y = clientY / h;
    /* Prefer the nearest edge so all four sides work */
    const dLeft = x;
    const dRight = 1 - x;
    const dTop = y;
    const dBottom = 1 - y;
    ...
    let best = "";
    let bestD = Infinity;
    if(dLeft <= limX && dLeft < bestD){ best = "left"; bestD = dLeft; }
    if(dRight <= limX && dRight < bestD){ best = "right"; bestD = dRight; }
    if(dTop <= limY && dTop < bestD){ best = "top"; bestD = dTop; }
    if(dBottom <= limY && dBottom < bestD){ best = "bottom"; bestD = dBottom; }
    return best;
  }
  ...
  function spawnPetal(layer, side, originX, originY, key){
    if(activeCount >= MAX_ACTIVE) return;
    const el = document.createElement("span");
    const depth = pick(["near", "mid", "far"]);
    const kind = depth === "far"
      ? (Math.random() < 0.45 ? "speck" : pick(["champagne", "rose", "blush"]))
      : pick(["ivory", "champagne", "rose", "crimson", "blush", "leaf", "sharp"]);

    el.className = "floral-petal depth-" + depth + (kind === "leaf" ? " is-leaf" : "") + (kind === "speck" ? " is-speck" : "");
    if(kind === "speck"){
      el.style.background = Math.random() < 0.55
    ...
    const w = (kind === "speck" ? 4 : kind === "leaf" ? 12 : 14) * sizeScale;
    const h = (kind === "speck" ? 4 : kind === "leaf" ? 20 : 18) * sizeScale;
    el.style.width = w + "px";
    el.style.height = h + "px";
    el.style.opacity = String(depth === "near" ? rand(0.55, 0.85) : depth === "mid" ? rand(0.35, 0.6) : rand(0.18, 0.38));

    const layerRect = layer.getBoundingClientRect();
    const layerW = layerRect.width || 1;
    const layerH = layerRect.height || 1;
    const touchX = originX - layerRect.left;

    /* Always start from TOP, spread by which edge was touched */
    let startX;
    if(side === "left") startX = rand(layerW * 0.02, layerW * 0.42);
    else if(side === "right") startX = rand(layerW * 0.58, layerW * 0.98);
    else if(side === "top") startX = touchX + rand(-layerW * 0.18, layerW * 0.18);
    else startX = touchX + rand(-layerW * 0.22, layerW * 0.22); /* bottom: still fall from top */
    startX = Math.max(4, Math.min(layerW - 4, startX));
    const startY = rand(-28, layerH * 0.06);

    el.style.transform = "translate3d(" + startX + "px," + startY + "px,0) rotate(" + rand(-30, 30) + "deg)";
    layer.appendChild(el);
    activeCount++;

    /* Fall downward with soft sideways sway — far travel */
    const fallY = rand(layerH * 0.55, layerH * 0.95);
    const sway = (Math.random() < 0.5 ? 1 : -1) * rand(36, 120);
    ...
    el.dataset.baseOp = String(baseOp);

    const t0 = performance.now();
    function frame(now){
      const p = Math.min(1, (now - t0) / dur);
      const ease = p * p * (3 - 2 * p); /* smooth fall */
      const wind = Math.sin(p * Math.PI * windFreq) * sway;
      const x = startX + wind * Math.sin(p * Math.PI);
      const y = startY + fallY * ease;
      /* Soft fade near bottom / deep center text band */
      const ny = y / layerH;
      let fade = 1;
      if(ny > 0.72) fade = Math.max(0, 1 - (ny - 0.72) / 0.28);
  ...
  function burst(side, clientX, clientY){
    if(document.body.classList.contains("gate-locked")) return;
    if(!inviteOpened) return;
    const now = Date.now();
    if(now - lastBurstAt < COOLDOWN_MS) return;
    const screen = visibleAllowedScreen();
    const key = screenKey(screen);
    if(!ALLOWED.has(key)) return;
    const cfg = BURST[key];
    const layer = ensureLayer(screen);
    const n = Math.round(rand(cfg.min, cfg.max + 0.49));
    lastBurstAt = now;
    for(let i = 0; i < n; i++){
      window.setTimeout(() => spawnPetal(layer, side, clientX, clientY, key), i * rand(8, 28));
    }
  }

  function onPointerDown(e){
    if(e.pointerType === "mouse" && e.button !== 0) return;
    if(isBlockedTarget(e.target)) { press = null; return; }
    const side = edgeSide(e.clientX, e.clientY);
    if(!side){ press = null; return; }
    press = { id: e.pointerId, x: e.clientX, y: e.clientY, side, t: Date.now() };
  }
  ...
  function onPointerUp(e){
    ...
    const side = press.side;
    const x = press.x;
    const y = press.y;
    press = null;
    if(dx > 12 || dy > 12) return; /* scroll / swipe */
    if(isBlockedTarget(e.target)) return;
    const sideNow = edgeSide(e.clientX, e.clientY);
    if(sideNow !== side && edgeSide(x, y) !== side) return;
    burst(side, x, y);
  }
  ...
  /* Prepare layers once for allowed screens */
  document.querySelectorAll("#invitation > .screen.host, #invitation > .screen.couple, #invitation > .screen.event.nikah, #invitation > .screen.event.walima").forEach(ensureLayer);
})();
