...
function finishAutoAtEnd(){
  ...
  /* After invitation ends → back to first page, auto-control again */
  window.setTimeout(restartInvitationFromStart, 1600);
}

function restartInvitationFromStart(){
  if(!inviteOpened || !autoScreens.length) return;
  if(document.hidden || document.visibilityState === "hidden") return;
  /* If guest already took manual control, don't force a loop */
  if(manualScrollMode) return;
  ...
  const y=first ? Math.max(0, Math.round(first.offsetTop || 0)) : 0;
  try{ window.scrollTo({top:y, left:0, behavior:"auto"}); }catch(_e){
  ...
  /* Resume music for the next loop */
  if(musicMode === "webaudio" && decodedBuffer){
    startWebAudioFromBuffer(decodedBuffer);
  }else{
    playHtmlAudio().then((ok)=>{
      if(!ok) playViaWebAudio();
    });
