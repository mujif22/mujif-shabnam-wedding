...
function applyHindiFonts(isHi){
  const targets=document.querySelectorAll(
    ".title-wedding,.title-invitation,.invite-open-hint,.host-hi-title,.host-families,.host-hi-date,.host-hi-day,.host-couple,.title-hero,.eyebrow,.event-meta,.event-date,.gold-btn,.closing h2,.closing-dates,[data-i18n='keep_duas'],.person-card h3,.person-card p,.translation,.blessing-note,.count-note,.couple-lead"
  );
  targets.forEach(el=>{
    if(isHi){
