...
function restoreOpenGate(){
  ...
  /* Refresh gate titles for current language */
  try{ setLanguage(currentLang); }catch(_e){}
  scheduleFitScreens();
}
...
function finishAutoAtEnd(){
  ...
  stopMusic();
  releaseScreenWakeLock();
  /* After full auto invitation ends → return to welcome gate (start page) */
  window.setTimeout(restoreOpenGate, 1200);
}
