...
(function initWeddingFloralInteraction(){
  ...
  function spawnPetal(layer, side, originX, originY, key){
    ...
    const startX = originX - layerRect.left + rand(-10, 10);
    const startY = originY - layerRect.top + rand(-12, 12);
    el.style.transform = "translate3d(" + startX + "px," + startY + "px,0) rotate(" + rand(-25, 25) + "deg)";
    layer.appendChild(el);
    activeCount++;

    /* Spread farther across the page — soft fade only near opposite side */
    const inward = side === "left" ? rand(110, 240) : -rand(110, 240);
    const up = -rand(36, 100);
    const driftY = rand(90, 210);
    const rot = (side === "left" ? 1 : -1) * rand(50, 170) + rand(-30, 30);
    const dur = (depth === "far" ? rand(2800, 4200) : depth === "mid" ? rand(2400, 3800) : rand(2200, 3600));
    const peak = 0.32;
    const windAmp = (side === "left" ? 1 : -1) * rand(14, 28);
    const windFreq = rand(1.05, 1.7);
    ...
    function frame(now){
      ...
      const ease = 1 - Math.pow(1 - p, 1.45);
      const wind = Math.sin(p * Math.PI * windFreq) * windAmp * (1 - p * 0.55);
      let x = startX + inward * ease + wind;
      const yRise = up * Math.sin(Math.min(1, p / peak) * Math.PI * 0.5);
      const yFall = driftY * Math.max(0, (p - peak) / (1 - peak));
      const y = startY + yRise + yFall;
      const layerW = layerRect.width || 1;
      const nx = x / layerW;
      let fadeEdge = 1;
      if(side === "left" && nx > 0.58) fadeEdge = Math.max(0, 1 - (nx - 0.58) / 0.28);
      if(side === "right" && nx < 0.42) fadeEdge = Math.max(0, 1 - (0.42 - nx) / 0.28);
      const op = baseOp * (1 - p * 0.88) * fadeEdge;
  ...
  function burst(side, clientX, clientY){
    ...
    lastBurstAt = now;
    for(let i = 0; i < n; i++){
      window.setTimeout(() => spawnPetal(layer, side, clientX, clientY, key), i * rand(18, 48));
    }
  }
  ...
  function onPointerUp(e){
    ...
    const y = press.y;
    press = null;
    if(dx > 10 || dy > 10) return; /* scroll / swipe */
    if(isBlockedTarget(e.target)) return;
    if(edgeSide(e.clientX) !== side && edgeSide(x) !== side) return;
    burst(side, x, y);
  }
