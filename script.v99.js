...
/* —— Soft floral petal interaction (all screens, fall from full top) —— */
(function initWeddingFloralInteraction(){
  if(reduceMotion) return;

  const DEFAULT_BURST = { min:14, max:22 };
  const BURST = {
    gate: { min:10, max:16 },
    host: { min:14, max:22 },
  ...
  function screenKey(screen){
    if(!screen) return "";
    if(screen.id === "openGate" || screen.classList.contains("welcome-gate") || screen.classList.contains("emerald-gate")) return "gate";
    if(screen.id === "host" || screen.classList.contains("host")) return "host";
    if(screen.classList.contains("blessing")) return "blessing";
    ...
    if(screen.classList.contains("countdown-screen")) return "countdown";
    if(screen.classList.contains("closing")) return "closing";
    if(screen.classList.contains("screen")) return "host";
    return "";
  }
  ...
  function activeScreen(){
    if(document.body.classList.contains("gate-locked")){
      return document.getElementById("openGate");
    }
    return visibleInviteScreen();
  }

  function isBlockedTarget(t){
    if(!t || !t.closest) return true;
    if(t.closest("#homeBackBtn, .home-back-btn, .language-btn, .gold-btn, a, button, .qr-card, .qr-card img, input, textarea, select, label, .lang-switch, audio")) return true;
    return false;
  }
  ...
  function spawnPetal(layer){
    ...
    function frame(now){
      ...
      const x = startX + Math.sin(p * Math.PI * windFreq) * sway * Math.sin(p * Math.PI);
      const y = startY + fallY * ease;
      const ny = y / layerH;
      let fade = 1;
      if(ny > 0.78) fade = Math.max(0, 1 - (ny - 0.78) / 0.28);
  ...
  function burst(){
    ...
    if(!layer) return;
    const cfg = BURST[key] || DEFAULT_BURST;
    const n = Math.round(rand(cfg.min, cfg.max + 0.49));
    lastBurstAt = now;
    for(let i = 0; i < n; i++){
      window.setTimeout(() => spawnPetal(layer), i * rand(8, 28));
    }
  }
  ...
  document.querySelectorAll("#invitation > .screen, #openGate").forEach(ensureLayer);
})();