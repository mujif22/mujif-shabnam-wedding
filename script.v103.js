...
(function initWeddingFloralInteraction(){
  ...
  const COOLDOWN_MS = 90; /* very soft/sensitive — light touch also fires */
  ...
  function spawnPetal(layer, direction){
    ...
    /* Full width: top→down OR bottom→up */
    ...
    function frame(now){
      ...
      const y = startY + travelY * ease;
      const ny = y / layerH;
      let fade = 1;
      if(fromBottom){
        if(ny < 0.22) fade = Math.max(0, ny / 0.22);
      }else if(ny > 0.78){
  ...
  function burst(){
    ...
    for(let i = 0; i < n; i++){
      ...
      window.setTimeout(() => spawnPetal(layer, dir), i * rand(8, 28));
    }
  }
  ...
  function onPointerDown(e){
    ...
    /* Fire on contact — soft/light touch, no need to press hard or wait for release */
    burst();
  }

  document.addEventListener("pointerdown", onPointerDown, { passive: true, capture: true });
  /* Extra mobile sensitivity: touchstart also (deduped by cooldown) */
