...
(function initWeddingFloralInteraction(){
  ...
  function spawnPetal(layer, direction){
    ...
    const kind = depth === "far"
      ? (Math.random() < 0.5 ? "speck" : pick(["champagne", "blush", "ivory"]))
      : pick(["ivory", "champagne", "rose", "blush", "leaf", "sharp"]);

    el.className = "floral-petal depth-" + depth + (kind === "leaf" ? " is-leaf" : "") + (kind === "speck" ? " is-speck" : "");
    if(kind === "speck"){
      el.style.background = Math.random() < 0.55
        ? "rgba(176,48,58," + rand(0.22, 0.4).toFixed(2) + ")"
        : "rgba(224,192,122," + rand(0.2, 0.38).toFixed(2) + ")";
    }else{
