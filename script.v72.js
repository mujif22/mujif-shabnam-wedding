...
/* Browser back → Wedding Invitation / जश्ने शादी gate */
function markHistoryGate(){
  try{
    const path=location.pathname + location.search;
    if(history.state && history.state.weddingView === "gate") return;
...
function markHistoryInvite(){
  try{
    const path=location.pathname + location.search;
    if(history.state && history.state.weddingView === "invite") return;
...
function onWeddingPopState(ev){
  ...
  if(view === "invite"){
    /* Forward navigation — keep invite if already open */
    return;
  }
  /* Back (or missing state) → language gate */
  if(inviteOpened || !document.body.classList.contains("gate-locked")){
    restoreOpenGate();
  }
}
