...
(function initWeddingFloralInteraction(){
  ...
  function spawnPetal(layer, side, originX, originY, key){
    ...
    const kind = depth === "far" ? (Math.random() < 0.55 ? "speck" : "champagne") : pick(["ivory", "champagne", "leaf", "sharp"]);
    ...
    const inward = side === "left" ? rand(28, 72) : -rand(28, 72);
    const up = -rand(24, 70);
    const driftY = rand(70, 150);
    const rot = (side === "left" ? 1 : -1) * rand(40, 140) + rand(-20, 20);
    const dur = (depth === "far" ? rand(2200, 3200) : depth === "mid" ? rand(1800, 2800) : rand(1600, 2500));
    const peak = 0.38;
    const windAmp = (side === "left" ? 1 : -1) * rand(8, 14);
    const windFreq = rand(1.1, 1.55);
    ...
    function frame(now){
      ...
      const ease = 1 - Math.pow(1 - p, 1.55);
      const wind = Math.sin(p * Math.PI * windFreq) * windAmp * (1 - p);
      ...
      /* Soft fade before deep center */
      const layerW = layerRect.width || 1;
      const nx = x / layerW;
      let fadeEdge = 1;
      if(side === "left" && nx > 0.30) fadeEdge = Math.max(0, 1 - (nx - 0.30) / 0.18);
      if(side === "right" && nx < 0.70) fadeEdge = Math.max(0, 1 - (0.70 - nx) / 0.18);
      const op = baseOp * (1 - p * 0.92) * fadeEdge;
