...
function fitAllPages(){
  ...
  if(gate){
    const sheet=gate.querySelector(".emerald-gate-sheet");
...
function setLanguage(lang){
  ...
  /* Force welcome titles so they never go blank after lang/fit races */
  const line1=document.querySelector(".emerald-gate .title-wedding, .welcome-gate .title-wedding");
  const line2=document.querySelector(".emerald-gate .title-invitation, .welcome-gate .title-invitation");
  if(line1){
    line1.textContent=dict.welcome_line1 || (next === "hi" ? "जश्ने" : "Wedding");
    line1.style.setProperty("opacity", "1", "important");
    line1.style.setProperty("visibility", "visible", "important");
    line1.style.setProperty("color", next === "hi" ? "#f0d78a" : "#f3e3b8", "important");
    line1.style.setProperty("-webkit-text-fill-color", next === "hi" ? "#f0d78a" : "#f3e3b8", "important");
  }
  if(line2){
    line2.textContent=dict.welcome_line2 || (next === "hi" ? "शादी" : "Invitation");
    line2.style.setProperty("opacity", "1", "important");
    line2.style.setProperty("visibility", "visible", "important");
    line2.style.setProperty("color", next === "hi" ? "#f0d78a" : "#e8d09a", "important");
    line2.style.setProperty("-webkit-text-fill-color", next === "hi" ? "#f0d78a" : "#e8d09a", "important");
  }
...
function pinLangSwitchOnGate(){
  if(!openGate) return;
  const switcher=document.getElementById("langSwitch");
  if(!switcher) return;
  switcher.classList.add("lang-switch-gate");
  switcher.classList.remove("is-fixed");
  const sheet=openGate.querySelector(".emerald-gate-sheet");
  if(switcher.parentNode !== openGate){
    openGate.insertBefore(switcher, sheet || openGate.firstChild);
  }
}
...
function hideOpenGate(){
  ...
  scheduleFitScreens();
  /* Keep gate in DOM so we can return to it after invitation ends */
}
...
function restoreOpenGate(){
  ...
  if(openGate){
    ...
    /* Force visible — some phones keep opacity:0 after .is-gone */
    ...
    pinLangSwitchOnGate();
    openGateHint=openGate.querySelector(".open-gate-hint");
    if(openGateHint) openGateHint.textContent=t("tap_to_open");
    document.body.classList.add("gate-locked");

    const dict=I18N[currentLang] || I18N.en;
    const line1=openGate.querySelector(".title-wedding");
    const line2=openGate.querySelector(".title-invitation");
    if(line1) line1.textContent=dict.welcome_line1 || (currentLang === "hi" ? "जश्ने" : "Wedding");
    if(line2) line2.textContent=dict.welcome_line2 || (currentLang === "hi" ? "शादी" : "Invitation");
  }
